...
public class 下一層時間_分 extends 下一層時間 implements 下一層時間計算 {
	// 主詞条Map
	//		詞条ID = 年
	//		数据ID = 2018
	...
	public List<Map> 取得下一層時間(Map 主詞条Map, Map w詞条Map) {
		...
		aCalendar.set(Calendar.HOUR_OF_DAY, Integer.parseInt(実体数据.substring(8, 9)));
		
		// 取得其下一级时间的最大值
		int maxMINUTE = aCalendar.getActualMaximum(Calendar.MINUTE);
		
		// 然后分别遍历取得具体值。分别存入
		for(int i=1;i<=maxMINUTE; i++) {
			String smaxMINUTE = 実体数据 + i;
			Map 子时间Map  = new HashMap();
			子时间Map.put("时间单位", w詞条Map.get(""));
			子时间Map.put("时间值", String.format("%tm", smaxMINUTE));
